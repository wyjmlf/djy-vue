<template>
  <div>
    ...
    >
      ...
      <br />
      <el-form-item>
        <el-button type="warning" @click="submitForm('ruleForm')">提交修改</el-button>
...
export default {
  ...
  methods: {
    /**获取角色详情 */
    getRoleInfo() {
      let url = "/role/findById";
      this.$axios.post(url, { id: this.ruleForm.id }).then(result => {
        if (result.data.code == 200) {
          this.layTree = result.data.data.roleMenu;
          this.ruleForm = result.data.data.roleInfo;
          console.log(this.layTree)
          this.treeNode();
        }
      });
    },
    ...
    //       }
    //     });
    //     tree.setChecked('demoId1', this.checkedList); //批量勾选 id 为 2、3 的节点
    //   });
    // },
